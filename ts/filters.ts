...
const displayProduct = () => {
  ...
  productContainer.innerHTML = filteredProducts
    .map(({ id, title, image, company, price }) => {
...
const formInput = document.querySelector(".input-form") as HTMLFormElement;
...
const companiesDOM = document.querySelector(".companies") as HTMLElement;
...
companiesDOM.addEventListener("click", (e: Event) => {
  const targetElement = e.target as HTMLElement;
  if (targetElement.dataset.id === "all") filteredProducts = [...products];
  else {
    filteredProducts = products.filter((product: product) => {
      return product.company === targetElement.dataset.id;
    });
  }
  searchInput.value = "";
  displayProduct();
